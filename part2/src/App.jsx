...
const App = () => {
  ...
  const inputChange_num = (event) => setNewNumber(event.target.value);
  const findName = (event) => setFinder(event.target.value);
  ...
  const Notification = ({ message, errcheck, vis }) => {
    ...
    return (
      <div style={NotStyles}>{message}</div>
    );
  };
    ...
    if (existingPerson) {
      if (window.confirm(`${newName} is already in the phonebook. Do you want to update their number?`)) {
        BackEnd.updatePerson(existingPerson.id, newName, newNumber).then(updatedPerson => {
          ...
          setTimeout(() => { setNotifMessage(null); setVisibility(false); }, 5000);
        }).catch(error => {
          setCheck(false);
          setVisibility(true);
  ...
  return (
    <div>
      <Notification message={errorMessage} errcheck={errcheck} vis={vis} />
      <h2>Phonebook</h2>
